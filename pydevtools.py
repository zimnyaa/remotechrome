    ...
    def __getattr__(self, attr):
        ...
        def generic_function(**args):
            ...
            if self.sessionId is None:
                ...
                call_obj = {'sessionId': self.sessionId, 'id': message_id, 'method': func_name, 'params': args}
            #print(self.sessionId, json.dumps(call_obj))
...
class ChromeInterface(object):
    ...
    def connect(self, tab=0, update_tabs=True):
        if update_tabs or self.tabs is None:
            self.get_tabs()
        wsurl = self.tabs['webSocketDebuggerUrl']
        self.close()
    ...
    def __getattr__(self, attr):
        ...
        #print("[+] creating ", attr, "with sessionID", self.sessionId)
        if self.sessionId is None:
            self.__setattr__(attr, genericelement)
            return genericelement
        else:
            self.__setattr__(attr, genericelement)
            genericelement.sessionId = self.sessionId
            return genericelement
